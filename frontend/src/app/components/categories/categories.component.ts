...
export class CategoriesComponent implements OnInit {
    ...
    search(pagination: Pagination = new Pagination) {
        ...
        this.inertia.getElements(this.objectName, (new Category).$, this.$filter, this.pagination).then((({data, pagination}) => {
    ...
    async remove() {
        const items: any = this.categories.filter((x: any) => x.$_select);
        ...
        this.inertia.saveElement(this.objectName, items).then((res: any) => {
            if (res) {
                this.ui.messageSuccess('Category deleted');
                this.search();
            }
        });
    }
    ...
    get hasBulkAction() {
        return this.categories.filter((x: any) => x.$_select).length;
    }
}
